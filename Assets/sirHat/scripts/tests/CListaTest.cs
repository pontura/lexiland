...
public class CListaTest
{
    ...
    private Dictionary<int, CEstimulo> _estimulos;
    ...
    // Constructor
    public CListaTest(string _pathInput, string _pathAudios, string _pathImages)
    {
        // Inicializamos
        _cantEjemplos = 0;
        _cantPractica = 0;
        ...
        _estimulos = new Dictionary<int, CEstimulo>();
        // Leemos el csv
        ...
        for (int _i = 1; _i < _lines.Length; _i++)
        {
            if (_lines[_i] != "")
            {
                CEstimulo _estimuloAux = new CEstimulo(_lines[_i], _pathAudios, _pathImages);
    ...
    public CEstimulo ObtenerEstimulo(int _ind)
    {
