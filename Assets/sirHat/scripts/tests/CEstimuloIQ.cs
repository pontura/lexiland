...
public class CEstimuloIQ
{
    private string _task, _trialCode, _target, _visual1, _visual2, _visual3, _visual4, _visual5, _visual6, _corrRespAux,_corrResp, _numSyll, _numPhon, _struct, _syllPos, _trialType, _audio1, _audio2;
    ...
    {
        string[] rowDataTemp = { "trialCode", "target", "visual1", "visual2", "visual3", "visual4","visual5","visual6", "corrResp", "corrResp", "numSyll", "numPhon", "struct",
            "syllPos", "trialType","audio1", "audio2", "numPrac", "trialPos", "targetPos", "resp", "acierto", "RT"};
        return rowDataTemp;
    }

    public string[] GetEstimulo(int _numPrac, int _trialPos, int _targetPos, string _tResp, string _respuesta, string _acierto)
    {
        string _resplog = _targetPos.ToString();
        if (_targetPos == -1)
            _resplog = "NA";

        string[] rowDataTemp = {  TrialCode, Target, Visual1, Visual2, Visual3, Visual4, Visual5, Visual6, CorrResp, CorrRespAux, NumSyll, NumPhon, Struct, SyllPos,
            TrialType, Audio1, Audio2, _numPrac.ToString(), _trialPos.ToString(), _resplog, _respuesta, _acierto, _tResp};
        return rowDataTemp;
    }


    // Constructor
    public CEstimuloIQ(string _linea, string _pathA, string _pathV)
    {
    ...
    private void CargarStrings(string _linea)
    {
        ...
        _visual5 = _lineaData[7];
        _visual6 = _lineaData[8];
        _corrRespAux =  _lineaData[9];
        _corrResp = _lineaData[10];
        _numSyll = _lineaData[11];
        _numPhon = _lineaData[12];
        _struct = _lineaData[13];
        _syllPos = _lineaData[14];
        _trialType = _lineaData[15];
        _audio1 = _lineaData[16];
        _audio2 = _lineaData[17];
    }
    ...
    public string CorrRespAux
    {
        get { return _corrRespAux; }
    }
