...
public class CListaTestIQ
{

    // Atributos
    private int _cantEjemplos, _cantPractica, _cantTest;
    private Dictionary<int, CEstimuloIQ> _estimulos;
    ...
    // Constructor
    public CListaTestIQ(string _pathInput, string _pathAudios, string _pathImages)
    {
        // Inicializamos
        _cantEjemplos = 0;
        _cantPractica = 0;
        ...
        _estimulos = new Dictionary<int, CEstimuloIQ>();
        // Leemos el csv
        ...
        for (int _i = 1; _i < _lines.Length; _i++)
        {
            if (_lines[_i] != "")
            {
                CEstimuloIQ _estimuloAux = new CEstimuloIQ(_lines[_i], _pathAudios, _pathImages);
    ...
    public CEstimuloIQ ObtenerEstimulo(int _ind)
    {
